...
using Hagar.Analyzers.Helpers.Comparers;
using Hagar.Analyzers.Helpers.Pooled;
using Microsoft.CodeAnalysis;
using Microsoft.CodeAnalysis.CSharp.Syntax;

namespace Hagar.Analyzers.Helpers.Walkers
{
            ...
            using (var pooledAssignments = Create(scope, search, semanticModel, cancellationToken))
            {
                foreach (var candidate in pooledAssignments.Item.Assignments)
                {
                    ...
                    if (candidate.Right is ObjectCreationExpressionSyntax objectCreation &&
                        ...
                        objectCreation.ArgumentList.Arguments.TryGetFirst(x => SymbolComparer.Equals(symbol, SemanticModelExt.GetSymbolSafe(semanticModel, (SyntaxNode) x.Expression, cancellationToken)), out ArgumentSyntax _))
                    {
                        assignment = candidate;
                        return true;
                    }
